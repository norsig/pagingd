...
sub sendRecentRooms($) {
    ...
    if ($#g_aRecentRooms >= 0) {
        foreach my $tR (reverse @g_aRecentRooms) {
            main::sendSmsPage($iSender, prettyDateTime($tR->{creation_time}, 1) . ": " . roomStatusIndividual(0, 0, 0, 0, $tR->{most_occupants_by_phone}) . "\n" . $tR->{summary} . "\n");
        }
    }
    else {
        main::sendSmsPage($iSender, t(S_NoRecent));
    }
}
